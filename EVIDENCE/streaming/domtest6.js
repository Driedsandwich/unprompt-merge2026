// 2026-07-30 追加の2機能を機械で押さえる(domtest2.js と同型の最小 DOM シム)。
//
//   A) カウンタ行の「すべて委ねる」
//      A-1 ストリーミング中(総数未確定)は出さない / 未決0でも出さない
//      A-2 一括委任 → カウンタ0 → 指示書ボタンが出る
//      A-3 1件を語クリックで選び直すと指示書ボタンが消え、「すべて委ねる」が戻る
//      A-4 一括委任の結果は「1件ずつ委ねた」結果と完全に一致する
//          (専用の状態遷移を作っていないこと = 既存の委任経路そのものであることの証明)
//
//   B) 「処理の内訳」パネル
...
(async () => {
  ...
  chk('ボタン文言は「すべて委ねる」', /<button id="btnDelegateAll"[^>]*>すべて委ねる<\/button>/.test(html));
  chk('「すべて委ねる」は既定で hidden', /<button id="btnDelegateAll"[^>]*\shidden[\s>]/.test(html));
  chk('カウンタと同じ行(#counterRow)に入っている',
      /<div id="counterRow">[\s\S]{0,400}?id="btnDelegateAll"[\s\S]{0,120}?<\/div>/.test(html));
  chk('カウンタが先・ボタンが後(= 行の右端)',
      html.indexOf('id="branchCounter"') < html.indexOf('id="btnDelegateAll"'));
  ...
  chk('ゴースト様式(枠線のみ・鼠・ホバー藍)',
      /#btnDelegateAll\{[^}]*background:transparent[^}]*border:1px solid var\(--line\)[^}]*color:var\(--muted\)/.test(html) &&
      /#btnDelegateAll:hover\{[^}]*color:var\(--ai\)[^}]*border-color:var\(--ai\)/.test(html));
  chk('角丸ゼロ', /#btnDelegateAll\{[^}]*border-radius:0/.test(html) && /#procPanel\{[^}]*border-radius:0/.test(html));
  ...
  EVENT_GAP_MS = 120;
  {
    ...
    chk('★ストリーミング中は「すべて委ねる」を出さない', registry['btnDelegateAll'].hidden === true);
    while (guard++ < 900 && ctx.__S.streamDone !== true){ flushRAF(2); await sleep(8); }
    flushRAF(4);
    chk('done 後は出る', registry['btnDelegateAll'].hidden === false);
  }
  EVENT_GAP_MS = 0;

  console.log('\n=== A-2) 一括委任 → カウンタ0 → 指示書ボタン ===');
  ...
  chk('未決ありなので「すべて委ねる」が出ている', registry['btnDelegateAll'].hidden === false);
  ...
  chk('★委任は oi を持たない(1件ずつと同じ形)', S.decisions.every(d => d.oi === null && d.prev === null));
  ...
  chk('★未決0なので「すべて委ねる」は消える', registry['btnDelegateAll'].hidden === true);
  chk('原文の語が全て鼠(委任)になる',
  ...
  chk('★カウンタの未決が 1 に戻る', registry['branchCounter'].textContent.indexOf('未決 1') > 0,
      JSON.stringify(registry['branchCounter'].textContent));
  chk('★「すべて委ねる」が戻る', registry['btnDelegateAll'].hidden === false);
