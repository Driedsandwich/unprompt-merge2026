#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v2: 見本つき選択の handoff_v2.json から compiled_v2 を生成し、
raw は v1 と同一物を再利用 (raw条件は選択に依存しないため・対照の固定)。
blind_v2/ は左右順のシードを変えて再ブラインドする。"""
...
SESS = HERE / "sessions"
PAIRS1 = HERE / "pairs"
PAIRS2 = HERE / "pairs_v2"
BLIND2 = HERE / "blind_v2"
MANIFEST = HERE / "measure_manifest_v2.json"


def blind_side(bid):
    return "AB" if int(hashlib.sha256((bid + ":v2").encode()).hexdigest(), 16) % 2 == 0 else "BA"


def main():
    runner = server.ClaudeRunner("claude", 180, 4, allow_api_key=False, effort="low")
    man = {"format": "unprompt.effect_measure.v2", "pairs": {}}
    for bid in sorted(SPECS):
        ...
        print(f"[{bid}] compiled_v2 生成中...", flush=True)
