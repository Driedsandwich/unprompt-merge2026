...
def load(j):
    return json.loads((HERE / "judgments" / f"judge{j}_v5.json").read_text(encoding="utf-8"))

def main():
    data = {j: load(j) for j in JUDGES}
    ids = sorted(MAN["pairs"])
    out = {"format": "unprompt.effect_tally.v5", "per_brief": {}, "summary": {}}
    ...
    (HERE / "tally_v5.json").write_text(json.dumps(out, ensure_ascii=False, indent=1),
                                        encoding="utf-8")
    s = out["summary"]
    print("== 測定v5 集計 (n=%d×判定3体) ==" % n)
