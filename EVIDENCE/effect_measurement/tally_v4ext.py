#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""測定v4 追加走の集計 (PREREGISTRATION.md 追補5)

judgments/ext/<model>_judge{1..3}_v4.json をモデルごとに v4 と同じ規則で集計し、
sonnet 本走 (tally_v4.json) との一致 (方向・ブリーフ別勝敗・捏造の重なり) と
costs_v4ext.jsonl の実費合計を出す。出力: tally_v4ext.json + 標準出力の要約。
"""
...
MAN = json.loads((HERE / "measure_manifest_v4.json").read_text(encoding="utf-8"))
BASE = json.loads((HERE / "tally_v4.json").read_text(encoding="utf-8"))
...
KNOWN_FABS = {
    ("m06-news", "compiled"): ["冷やし飴"],
    ("m07-recruit", "compiled"): ["3か月", "3ヶ月", "三か月"],
    ("m06-news", "raw"): ["8月"],
}
...
def tally_model(short):
    ...
    for j in JUDGES:
        ...
        for metric in ("rework", "fabrication"):
            p = EXT / f"{short}_judge{j}_{metric}_v4.json"
            if not p.exists():
                continue
...
def costs():
    ...
    calls = 0
    errors = 0
    if (EXT / "costs_v4ext.jsonl").exists():
        for line in (EXT / "costs_v4ext.jsonl").read_text(encoding="utf-8").splitlines():
            rec = json.loads(line)
...
def main():
    out = {"format": "unprompt.effect_tally.v4ext", "models": {}, "costs": costs()}
    for short in MODELS:
        m = tally_model(short)
        m["concordance_vs_sonnet"] = concordance(m)
        out["models"][short] = m
    (HERE / "tally_v4ext.json").write_text(json.dumps(out, ensure_ascii=False, indent=1),
                                           encoding="utf-8")
    print("== 測定v4 追加走 (複数モデル判定) 集計 ==")
    for short in MODELS:
        m = out["models"][short]
