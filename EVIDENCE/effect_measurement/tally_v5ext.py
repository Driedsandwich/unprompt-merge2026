#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""測定v5 外部3モデル判定の集計 (PREREGISTRATION.md 追補6 手続き4b)

judgments/ext_v5/<model>_judge{1..3}_{metric}_v5.json をモデルごとに v4 と同じ規則で集計し、
sonnet 本走 (tally_v5.json) との一致 (方向・ブリーフ別勝敗・捏造の重なり) と
costs_v5ext.jsonl の実費合計を出す。出力: tally_v5ext.json + 標準出力の要約。
"""
...
EXT = HERE / "judgments" / "ext_v5"
MAN = json.loads((HERE / "measure_manifest_v5.json").read_text(encoding="utf-8"))
BASE = json.loads((HERE / "tally_v5.json").read_text(encoding="utf-8"))
...
def tally_model(short):
    ...
    for j in JUDGES:
        ...
        for metric in ("rework", "fabrication"):
            p = EXT / f"{short}_judge{j}_{metric}_v5.json"
            if not p.exists():
                continue
...
def costs():
    ...
    calls = 0
    errors = 0
    if (EXT / "costs_v5ext.jsonl").exists():
        for line in (EXT / "costs_v5ext.jsonl").read_text(encoding="utf-8").splitlines():
            rec = json.loads(line)
...
def main():
    out = {"format": "unprompt.effect_tally.v5ext", "models": {}, "costs": costs()}
    for short in MODELS:
        m = tally_model(short)
        m["concordance_vs_sonnet"] = concordance(m)
        out["models"][short] = m
    (HERE / "tally_v5ext.json").write_text(json.dumps(out, ensure_ascii=False, indent=1),
                                           encoding="utf-8")
    print("== 測定v5 外部3モデル判定 集計 ==")
    for short in MODELS:
        m = out["models"][short]
