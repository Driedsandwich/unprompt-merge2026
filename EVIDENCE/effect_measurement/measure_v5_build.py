#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""測定v5 構築ランナー (PREREGISTRATION.md 追補6)

v4 から固定再利用: sessions/*/handoff_v4.json・brief・raw 成果物 (pairs_v4/*/raw.html)。
新規: compiled のみ二律入り COMPILED_EXTRA (scripts/pregen_compare.py・コミット済み) で12本再生成。

工程:
  --artifacts : pairs_v5/<id>/{raw.html(コピー), compiled.html(新規)} + measure_manifest_v5.json
  --blind     : blind_v5/<id>/art_1.html, art_2.html (割当シード ":v5-single"・対応表は manifest のみ)
"""
...
PAIRS5 = HERE / "pairs_v5"
BLIND5 = HERE / "blind_v5"
MANIFEST = HERE / "measure_manifest_v5.json"


def do_artifacts(runner, model):
    man = {"format": "unprompt.effect_measure.v5", "pairs": {}}
    if MANIFEST.exists():
...
def art_order(bid):
    h = int(hashlib.sha256((bid + ":v5-single").encode()).hexdigest(), 16)
...
def do_blind():
    ...
    MANIFEST.write_text(json.dumps(man, ensure_ascii=False, indent=1), encoding="utf-8")
    print("blind_v5 構築完了 (対応表は manifest のみ)")
