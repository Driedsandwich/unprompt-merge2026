...
def load(j):
    return json.loads((HERE / "judgments" / f"judge{j}_v4.json").read_text(encoding="utf-8"))

def main():
    data = {j: load(j) for j in JUDGES}
    ids = sorted(MAN["pairs"])
    out = {"format": "unprompt.effect_tally.v4", "per_brief": {}, "summary": {}}
    ...
    (HERE / "tally_v4.json").write_text(json.dumps(out, ensure_ascii=False, indent=1),
                                        encoding="utf-8")
    s = out["summary"]
    print("== 測定v4 集計 (n=%d×判定3体) ==" % n)
