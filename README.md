...
```mermaid
flowchart LR
    A["曖昧な一文"] --> B["爆散<br/>3〜5の判断点"] --> C["決める / 委ねる<br/>(クリックのみ)"] --> D["意図の指示書<br/>(JSON)"] --> E["いつものAIへ<br/>ChatGPT / Claude / 画像 / 動画"]
```
...
必要なもの: **Claude Code にログイン済みの環境だけ** (APIキー不使用・npm/ビルド不要・Python標準ライブラリのみ)。ホームの例文チップ7種 (会社のLP / イベント告知 / スカウトメール / 商品の説明文 / ポートフォリオ / キービジュアル / 紹介動画) からワンクリックで始められます。
...
| どの商品にもなり得る一枚 | ギフト・重厚・人物と共に — 決めた4つの判断点が全部絵に出る |
