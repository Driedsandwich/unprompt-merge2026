...
server.py は静的ファイル(app/ 配下)を `/` で配信しつつ、`/api/explode` `/api/render` `/api/compile` `/api/health` を提供する。localhost 以外へはバインドしない。

### ゴールデンパス(操作手順)

1. **初期画面**: 見慣れたプロンプト箱だけ。ブリーフ文を入力して送信する。人間がタイプするのはこの最初の一文のみ(以降タイプしない)。
2. **静寂**: 送信直後、生成は始まらない。0.1秒刻みの経過秒カウンタと極小文字「x1.0 — 生成は、始まらない」が表示される。これは演出ではなく実測時間そのもの。
3. **爆散**: 文が語スパンに分解され、各判断点カードが anchor 語と SVG テザー線で結ばれて周囲に配置される(このアニメーションは1回のみ)。カードは最大5枚、ミニレンダはトークン到着まではスケルトン表示(白紙禁止)。
4. **カード操作(二動詞のみ)**:
...
すべてのAPI呼び出し(`/api/explode` `/api/render` `/api/compile`)は `logs/session_<起動時刻>.jsonl` に1行1呼び出しで追記される。各行は `{ts, endpoint, model, wall_ms, api_ms, ok}` を含む。サーバ起動ごとに新しいファイルが作られる(既存ファイルへの上書きなし)。
