...
- 「トーン例」の文体をコピーの基調にする。
- 決定は「実装」せよ、ただし「語る」な: 判断点名・選択ラベル・「〜向け」「〜重視」等の
  決定を説明するメタな言い回しを、見出し・本文・キャッチコピーなど読者に見える文面へ
  そのまま書かないこと。決定は配色・構成・語り口・内容の選択として現れ、
  ラベル文字列はコメント(機械照合用)にだけ残す。読者は発注の存在を知らない一般読者である。
...
- 固有のエピソード・具体的事実 (日付・固有名・数値・実績・談話) は創作せず、プレースホルダ〈〉にする。
  期間・年数・回数の表現 (「十年ぶり」「創業〇年」等) も数値の創作に含まれる。決まっていなければ〈年数〉ぶり のように書く。
- ただし条件・手順の骨子は、参照ページへ丸投げせずその場に実質を書く (値が未確定の箇所だけを〈〉にする)。
- プレースホルダ〈〉のうち、コンパイル済みブリーフの decisions の intent が名指しする内容 (例: 社員の生の言葉・失敗談・味や製法の具体・条件の実値) に当たるものは、単語の穴埋め形 (〈職種〉〈大事にしていること〉) にせず、発注者に何を確認したいかが分かる質問形 (〈確認: 現場で実際にあった失敗談を1つ〉) で書く。
"""
...
BRIEF_SPECS = [
    ...
    {
        "id": "keyvisual",
        "text": "新商品のキービジュアルを画像生成AIで作りたい。高級感があって、思わず目を引く感じで。",
        "kind": "text",
        "maker": "画像生成AIに渡すプロンプトを設計する専門家",
        "deliverable": "画像生成AIへ渡すプロンプト一式(本プロンプト+ネガティブ/スタイル指定+バリエーション指示)",
        "structure": "成果物の見出し / この絵で狙うこと(2〜3行) / "
                     "本プロンプト(そのままコピーして貼れる1ブロック。被写体・構図・画角・"
                     "光・質感・色・背景・雰囲気をこの順で書き切る) / "
                     "ネガティブプロンプト(避けたい要素の列挙) / "
                     "スタイル指定(参照する画作り・アスペクト比・解像度の目安) / "
                     "バリエーション指示3案(各1行。本プロンプトのどこを差し替えるかを書く)",
        "volume": "本プロンプト 150〜300 字、全体で 600〜900 字程度",
        "placeholder": "〈商品名〉〈ブランド名〉",
        # ギフト検討者 × 重厚クラシック × 世界観・情景演出 × 人物と共に提示。
        # 「高級感」を素で受けた RAW は「黒背景・余白多め・商品単体のクローズアップを
        # ミニマルに」という画像生成AIの既定解に落ちやすい。4点すべてでそこから離す。
        "picks": [2, 1, 2, 2],
        "expected_labels": ["ギフト検討者向け", "重厚・クラシック", "世界観・情景演出",
                            "人物と共に提示"],
    },
    {
        "id": "intro-video",
        "text": "会社紹介の30秒動画をAIでつくりたい。テンポよく、若い人にも刺さるように。",
        "kind": "text",
        "maker": "動画生成AIに渡す絵コンテとプロンプトを設計する専門家",
        "deliverable": "動画生成AIへ渡す絵コンテとプロンプト一式(ショット割り+各ショットのプロンプト)",
        "structure": "成果物の見出し / 全体設計(30秒の尺配分と狙いを2〜3行) / "
                     "ショット割り(5〜7ショット。1ショットにつき「番号・秒数・画で見せること」の1行と、"
                     "そのままコピーして貼れる生成プロンプト1ブロックを続けて置く) / "
                     "テロップと音の指定 / つなぎとテンポの注意",
        "volume": "ショットごとのプロンプトを含めて全体で 700〜1000 字程度",
        "placeholder": "〈会社名〉〈事業内容〉",
        # 就活層 × 採用ブランディング × 余白重視 × 社員/カルチャー × エモーショナル。
        # RAW は「テンポよく」を字面どおり高速カットで受け、事業紹介をSNS風ポップに流す
        # 汎用会社紹介に落ちやすい。「テンポ」を間の設計として読み替える第3候補が
        # いちばん遠く、採用・カルチャー・共感の3点と組んで1本の筋になる。
        "picks": [0, 0, 2, 2, 0],
        "expected_labels": ["就活/求職中の若者", "採用ブランディング", "情報量を絞り余白重視",
                            "社員/カルチャー訴求", "エモーショナル/共感型"],
    },
]
